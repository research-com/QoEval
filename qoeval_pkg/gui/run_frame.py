...
import qoeval_pkg.netem.netem as netem

import qoeval_pkg.gui.gui
from qoeval_pkg.coordinator import Coordinator, FINISH_CAMPAIGN_LOG, FINISH_POST_LOG
import threading
...
class RunFrame(tk.Frame):
    ...
    def __init__(self, master, gui: Gui, get_checked_entries: Callable[[], List[str]]):
        ...
        self.checkbox_frame = CheckboxToBooleanFrame(self, self.gui,
                                                     config_variables=
                                                     [self.gui.qoeval_config.coordinator_generate_stimuli,
                                                      self.gui.qoeval_config.coordinator_postprocessing,
                                                      self.gui.qoeval_config.coordinator_overwrite],
                                                     name="Options",
    ...
    def terminate_coordinator(self):
        ...
        if self.coordinator_is_running:
            ...
            netem.reset_device_and_ifb(self.gui.qoeval_config.net_device_name.get())
            self.enable_interface_after_coordinator()
            self.coordinator_is_running = False

    def start_coordinator(self):
        """Save the current config and start the coordinator"""
        entries = self.gui.qoeval_config.gui_coordinator_stimuli.get()
        if len(entries) < 1:
