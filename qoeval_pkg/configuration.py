...
from qoeval_pkg.gui.tooltip_strings import add_tooltips

# default file name of configuration file and mandatory section name
qoeval_CONF = 'qoeval.conf'
qoeval_SECTION = 'qoeval'
NETEM_SECTION = 'NETEM'
VALUE_SEPERATOR = ";"

GUI_DEFAULT_CONFIG_FILE = "qoeval_gui_default.conf"
GUI_DEFAULT_CONFIG_FILE_LOCATION = os.path.join(os.path.expanduser("~/.config/qoeval/"), GUI_DEFAULT_CONFIG_FILE)
...
_default_avd_path = os.path.join(pathlib.Path.home(), 'qoeval_avd')
_default_video_capture_path = os.path.join(pathlib.Path.home(), 'stimuli')

_default_config_file_locations = [os.path.join(os.path.dirname(__file__), qoeval_CONF),
                                  f'./{qoeval_CONF}', os.path.join(pathlib.Path.home(), qoeval_CONF),
                                  ]

if os.environ.get("qoeval_CONF"):
    _default_config_file_locations.append(os.environ.get("qoeval_CONF"))


class QoEvalConfiguration:
    """ This class reads a qoeval Config file, provides setters and getters for the options and allows saving of a config
    """
    ...
    def _get_config_parser(config_file_path: str = None) -> configparser.ConfigParser:
        ...
        if qoeval_SECTION not in parser:
            raise RuntimeError(
    ...
    def store_netem_params(self, emulation_parameters):
        for p in emulation_parameters:
            ...
            elif isinstance(emulation_parameters[p], int):
                IntOption(self, p, 0.0, NETEM_SECTION).set(emulation_parameters[p])
            else:
                Option(self, p, "", NETEM_SECTION).set(str(emulation_parameters[p]))


class Option:
    def __init__(self, config: QoEvalConfiguration, option: str, default, section: str = qoeval_SECTION,
                 expand_user: bool = False):
        self.config: QoEvalConfiguration = config
...
class BoolOption(Option):
    def __init__(self, config: QoEvalConfiguration, option: str, default: bool, section: str = qoeval_SECTION):
...
class IntOption(Option):
    def __init__(self, config: QoEvalConfiguration, option: str, default: int, section: str = qoeval_SECTION):
...
class FloatOption(Option):
    def __init__(self, config: QoEvalConfiguration, option: str, default: float, section: str = qoeval_SECTION):
        super().__init__(config, option, default, section)
...
class MobileDeviceTypeOption(Option):
    def __init__(self, config: QoEvalConfiguration, option: str, default: str, section: str = qoeval_SECTION):
        super().__init__(config, option, default, section)
...
class ListIntOption(Option):
    def __init__(self, config: QoEvalConfiguration, option: str, default: List[int], section: str = qoeval_SECTION):
        super().__init__(config, option, str(default), section)
...
class ListFloatOption(Option):
    def __init__(self, config: QoEvalConfiguration, option: str, default: List[float], section: str = qoeval_SECTION):
        super().__init__(config, option, str(default), section)
...
class ListOption(Option):
    def __init__(self, config: QoEvalConfiguration, option: str, default: List[str], section: str = qoeval_SECTION):
        super().__init__(config, option, str(default), section)
        self.default = default
...
class ListDictOption(Option):
    def __init__(self, config: QoEvalConfiguration, option: str, default: List[Dict], section: str = qoeval_SECTION):
        super().__init__(config, option, str(default), section)
