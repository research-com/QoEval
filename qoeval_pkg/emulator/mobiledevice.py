...
def adb_name(qoeval_config: QoEvalConfiguration):
    if len(qoeval_config.adb_device_serial.get()) > 1:
        return f"adb -s {qoeval_config.adb_device_serial.get()}"  # -e selects emulator, -d usb-connected device, -s serialnr
    else:
        return "adb"
...
class MobileDevice:
    ...
    def input_keyevent(self, keyevent: int):
        subprocess.run(shlex.split(f"{adb_name(self.qoeval_config)} shell input keyevent {keyevent}")).check_returncode()
    ...
    def get_ip_address(self) -> ipaddress:
        output = subprocess.run(shlex.split(
            f"{adb_name(self.qoeval_config)} shell ifconfig wlan0"),
    ...
    def measure_rtt(self) -> float:
        ...
        subprocess.run(shlex.split(f"{adb_name(self.qoeval_config)} shell ping -c 1 {MEASUREMENT_TEST_HOST}"),
                       stdout=subprocess.PIPE)
        # now perform the actual measurement
        output = subprocess.run(shlex.split(
            f"{adb_name(self.qoeval_config)} shell ping -c {MEASUREMENT_DURATION/0.2} -i 0.2 {MEASUREMENT_TEST_HOST}"),
            stdout=subprocess.PIPE,
            universal_newlines=True)
