...
DEVICE_NAME = "pixel"
VD_NAME = "qoeval_" + DEVICE_NAME + "_" + TARGET_NAME.replace("-", "_") + "_x86"
...
def avd_ini_file(qoeval_config: QoEvalConfiguration):
    return f"{qoeval_config.vd_path.get()}/config.ini"


def is_avd_config_readable(qoeval_config: QoEvalConfiguration) -> bool:
    if os.access(f"{avd_ini_file(qoeval_config)}", os.R_OK):
        return True
    else:
        return False
...
class StandardEmulator(MobileDevice):
    ...
    def __write_avd_config(self):
        ...
        with open(f"{avd_ini_file(self.qoeval_config)}", 'w') as f:
    ...
    def __read_avd_config(self):
        ...
        with open(f"{avd_ini_file(self.qoeval_config)}", 'r') as f:
    ...
    def create_device(self, playstore=False):
        ...
        output = subprocess.run(shlex.split(
            f"{VD_MANAGER_NAME} create avd --package {package} --path {self.qoeval_config.vd_path.get()} " +
    ...
    def set_orientation(self, orientation: MobileDeviceOrientation):
        ...
        if self.qoeval_config.show_device_frame.get():
    ...
    def launch(self, orientation=MobileDeviceOrientation.PORTRAIT, playstore=False):
        ...
        if not is_avd_config_readable(self.qoeval_config):
    ...
    def shutdown(self):
        ...
        subprocess.run(shlex.split(f"{adb_name(self.qoeval_config)} emu kill"))


def main():
    print("Emulator control")
    emu = StandardEmulator(QoEvalConfiguration())
    # emu.delete_vd()
