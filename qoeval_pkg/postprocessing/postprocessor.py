...
from qoeval_pkg.videos import t_init
...
class PostProcessor:
    ...
    def process(self, input_filename: str, output_filename: str, initbuf_len: float, main_video_start_time: float,
        ...
        if _is_video_landscape(f"{os.path.join(self.qoeval_config.video_capture_path.get(), input_filename)}.avi"):
        ...
        if normalize_audio:
            target_volume = self.qoeval_config.audio_target_volume.get()
            current_volume = _get_max_volume(f"{os.path.join(self.qoeval_config.video_capture_path.get(), input_filename)}.avi")
        ...
        with importlib_resources.as_file(self._prefix_video) as prefix_video_path:
            ...
            if initbuf_len > 0:
                ...
                command = f"{FFMPEG} -i {prefix_video_path} " \
                          f"-i {os.path.join(self.qoeval_config.video_capture_path.get(), input_filename)}.avi " \
                          ...
                          f"-map \"[outv]\" -map \"[outa]\" " \
                          f" -y {os.path.join(self.qoeval_config.video_capture_path.get(), output_filename)}.avi"
            else:
                command = f"{FFMPEG} " \
                          f"-i {os.path.join(self.qoeval_config.video_capture_path.get(), input_filename)}.avi " \
                          f"-c:v mpeg4 -vtag xvid -qscale:v 1 -c:a libmp3lame -qscale:a 1 " \
            ...
            if initbuf_len > 0:
                command = f"{FFMPEG} -i {prefix_video_path} " \
                          ...
                          f"[v0][a0][v1][a1]concat=n=2:v=1:a=1[outv][outa]\" " \
                          f"-map \"[outv]\" -map \"[outa]\" " \
                          f" -y {os.path.join(self.qoeval_config.video_capture_path.get(), output_filename)}.mp4"
            else:
                command = f"{FFMPEG} " \
                          f"-i {os.path.join(self.qoeval_config.video_capture_path.get(), input_filename)}.avi " \
                          f"-crf 4 " \
...
def main():
    p = PostProcessor(QoEvalConfiguration())
