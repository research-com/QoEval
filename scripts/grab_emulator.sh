...
# Based on: https://github.com/AlbertVeli/grab_x11_window/blob/master/record_object.sh (no copyright)
...
# CMD="ffmpeg -f x11grab -r 25 -s ${width}x${height} -i :1${corner} $snd $vcodec -threads 0 output.mp4"
# CMD="ffmpeg -f pulse -thread_queue_size 512-i 6 -f x11grab -r 30 -s ${width}x${height} -i :1${corner} output.mp4"
CMD="ffmpeg -f x11grab -r 30 -s ${width}x${height} -i :1${corner} output.mp4"
exec $CMD